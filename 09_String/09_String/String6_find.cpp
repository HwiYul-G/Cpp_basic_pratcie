/*
	insert, erase, find는 문자열 연산 중 빈번히 사용된다.
	따라서 find 함수의 구현에 따라 문자열 클래스의 전반적인 성능이 좌우된다.

	왜냐하면 insert, erase는 연산 시간이 그렇게 오래 걸리지 않지만,
	문자열의 크기가 매우 크다면 find 연산은 매우 오래 걸릴 수 있기 때문이다.

	문자열을 검색하는 알고리즘은 매우 많지만
	어떤 상황에서도 좋은 성능을 발휘하는 알고리즘은 없다.
	(예를 들어 짧은 문자열 검색에 최적화된 알고리즘과 긴 문자열 검색에 최적화 된 알고리즘들 같다)
	그렇기에 특별한 알고리즘을 사용하는 경우에는 그 클래스의 사용 목적이 명확해서
	그 알고리즘이 좋은 성능을 발휘할 수 있는 경우에만 사용하는 것이 보통이다.

	따라서 우리의 MyString의 경우 가장 간단한 방법으로 find 알고리즘을 구현하자!
*/
