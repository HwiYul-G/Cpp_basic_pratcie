/*
	insert와 반대의 역할을 하는 erase 함수를 만들어보자.
	erase 함수는 insert 함수보다 만들기 쉬운데 왜냐하면 기본적으로 데이터의 양이 줄어들기 때문에
	복잡하게 capacity 이런 것들을 생각할 필요가 없기 때문이다.

	MyString& erase(int loc, int num);

	erase 함수는 위와 같이 생겼고
	loc은 insert와 동일하게 loc의 해당하는 문자 앞을 의미한다.
	그리고 num은 지우는 문자의 수를 의미한다.
	Ex) abcd에서 erase(1,2);을 하면 
	1에 해당하는 문자 'b'의 앞부터 2문자를 지우게 되어 bc가 지워지고 ad가 리턴된다.
*/
#include <iostream>
